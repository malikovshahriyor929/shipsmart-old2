      ...
      <div className="flex h-full min-h-[128px] flex-col items-center justify-center">
        ...
        {showLoadingText ? (
          <Title as="h6" className="-me-2 mt-4 font-medium text-gray-500">
            {t("commons.loading-ellipsis") ?? "Loading..."}
          </Title>
        ) : null}
      </div>
    );
    ...
    <>
        ...
        >
          <Fragment>
            {components?.header ? (
              ...
              <Table.Header
                className={cn(
                  stickyHeader && "sticky top-0 z-10 mb-4",
      ...
      {isEmpty(table.getRowModel().rows) && (
        <Box className="py-5 text-center lg:py-8">
          <Empty /> <Text className="mt-3">{t("commons.no-data") ?? "No Data"}</Text>
        </Box>
      )}
    </>
  );
            ...
            <Box className="flex items-start">
              ...
              {header.column.getCanSort() ? (
                <button
                  ...
                  aria-label={t("commons.sort-column") ?? "Sort Column"}
                >
                  {{
