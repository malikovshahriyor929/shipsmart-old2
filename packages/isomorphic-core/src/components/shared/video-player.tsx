...
export default function CustomVideoPlayer({
  ...
  // quality handling
  const initialIndex = useMemo(() => (sources?.length ? 0 : -1), [sources]);
  const [sourceIndex, setSourceIndex] = useState(initialIndex);
  const activeSrc = useMemo(
    () => (sources?.length ? sources[sourceIndex]?.url : src || ''),
    [src, sources, sourceIndex]
  ...
  // ===== events: metadata/time/buffer/fs/ended =====
  useEffect(() => {
    const v = videoRef.current;
    if (!v) return;
    ...
    const onTimeUpdate = () => {
      ...
      // raw time feed (for continuous-watch logic)
    ...
    const onKey = (e: KeyboardEvent) => {
      ...
      switch (e.key.toLowerCase()) {
        ...
        case 'k':
          ...
          break;
        case 'arrowright':
          setCurrentTime((v.currentTime || 0) + 5);
          break;
        case 'arrowleft':
          setCurrentTime((v.currentTime || 0) - 5);
          break;
    ...
    >
      ...
      {!playing && (
        <button
          aria-label="Play"
          onClick={togglePlay}
          className={`absolute inset-0 m-auto flex h-16 w-16 items-center justify-center rounded-full bg-white/10 text-white backdrop-blur-md transition hover:bg-white/20 ${size === 'sm' ? '!size-12' : ''}`}
        >
          <PiPlay className={`h-8 w-8 ${size === 'sm' ? '!size-8' : ''}`} />
        </button>
      )}
      ...
      >
        ...
        <div
          ...
          onPointerUp={endScrub}
          onClick={handleProgressPointer}
          className="mx-3 mt-2 h-2 cursor-pointer rounded-full bg-white/10"
        >
          {/* buffered */}
          <div
            className="h-2 rounded-full bg-white/25"
          ...
          {/* played */}
          <div
            className="relative -mt-2 h-2 rounded-full bg-mainBlue"
            style={{ width: `${progress * 100}%` }}
          >
            <div className="absolute -right-1 top-1/2 h-3 w-3 -translate-y-1/2 rounded-full bg-white shadow" />
          </div>
        ...
        {/* Buttons Row */}
        <div
          className={`flex items-center justify-between px-3 py-2 text-white ${size === 'sm' ? '!py-0' : ''}`}
        >
          {/* left cluster */}
          <div className="flex items-center gap-2">
            <button
              aria-label={playing ? 'Pause' : 'Play'}
              onClick={togglePlay}
              className={`rounded p-2 hover:bg-white/10 ${size === 'sm' ? 'hidden' : ''}`}
            >
              {playing ? (
                <PiPause className="h-6 w-6" />
              ) : (
            ...
            {/* volume */}
            <div
              className={`flex items-center ${size === 'sm' ? 'hidden' : ''}`}
            >
              <button
                aria-label={muted ? 'Unmute' : 'Mute'}
                onClick={() => setMuted((m) => !m)}
              ...
              <input
                ...
                className={`h-1 w-24 cursor-pointer accent-white ${size === 'sm' ? 'w-16' : ''}`}
              />
            </div>
          </div>
      ...
      {/* subtle hover curtain for responsive feel */}
      <div
        className={cn(
          'pointer-events-none absolute inset-0 transition-opacity',
          hovering || !playing ? 'opacity-100' : 'opacity-0'
