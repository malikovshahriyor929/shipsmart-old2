...
import { Fragment, useEffect, useRef, useState, ReactElement } from "react";
...
export default function SearchList({ onClose }: { onClose?: () => void }): ReactElement {
    ...
    <>
      <div className="flex items-center px-5 py-4">
        <Input
          ...
          placeholder={t("search.search-pages-here") ?? "Search pages here"}
          className="flex-1"
        ...
        <>
          {menuItemsFiltered.length === 0 ? (
            <Empty
              ...
              text={t("search.empty") ?? "No Result Found"}
            ...
            >
              {t("search.quick-page-links") ?? "Quick Page Links"}
            </Title>
          )}
        </>
