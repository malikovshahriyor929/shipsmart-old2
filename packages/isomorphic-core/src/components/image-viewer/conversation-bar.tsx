      ...
      <div className="grow divide-y space-y-4 px-6 pb-6">
        {comment.length < 1 && (
          <div className="text-center pt-6">
            ...
            <Text className="text-2xl text-gray-300">{t('chat.no-comments-yet') ?? 'No comments yet'}</Text>
          </div>
        )}
        {comment?.map((c) => (
          <div key={c.id} className="pt-4 group">
            <div className="flex items-start justify-between mb-3">
              ...
              />
              <Tooltip placement="left" content={t('chat.mark-as-resolved') ?? 'Mark as Resolved'}>
                <ActionIcon
                  size="sm"
                  variant="text"
  ...
  return (
    ...
    >
      <Input
        className="grow"
        name="comment"
        label={t('chat.add-a-comment') ?? 'Add a comment'}
        placeholder={t('chat.write-your-comment') ?? 'Write your comment...'}
      />
      <Button type="submit">
